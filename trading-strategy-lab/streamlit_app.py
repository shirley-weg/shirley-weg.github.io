...
@dataclass(frozen=True)
class CointegrationResearchConfig:
    period: str
...
def main() -> None:
    ...
    if st.session_state["selected_strategy"] == "strategy_2":
        st.caption("策略2：Cointegration Pairs 台股實證，從細產業 universe 自動篩選共整合 pair 並做 out-of-sample 回測。")
        settings = sidebar_strategy2_settings()
        render_cointegration_research(settings)
        return
    ...
    with col2:
        st.markdown(
            ...
            <div class="strategy-card">
              ...
              <p>
                保留台股單股期貨 universe，依細產業自動篩選高相關與
                ADF 通過的共整合 pair，並執行 formation / test 實證回測。
              </p>
            </div>
            """,
            unsafe_allow_html=True,
...
def render_cointegration_research(settings: dict[str, object]) -> None:
    ...
    st.write(
        "此策略會保留你的台股單股期貨選股池，先用 formation period 篩選共整合 pair，"
        "再在 test period 使用 rolling OLS z-score 做 out-of-sample 研究型回測。"
    )
    st.warning(
        "此版本為研究型 close-to-close 回測。尚未完整處理融券限制、借券成本、漲跌停、流動性衝擊與實際撮合滑價。",
        icon="⚠️",
    )
...
def run_strategy2_cointegration_research(
    ...
    for _, row in selected_pairs.iterrows():
        ...
        if not equity.empty:
            pnl_frames.append(equity["daily_pnl"].rename(pair_key))
    ...
    portfolio = build_strategy2_portfolio(pnl_frames, config.capital_per_pair)
...
def build_strategy2_portfolio(pnl_frames: list[pd.Series], capital_per_pair: float) -> pd.DataFrame:
    if not pnl_frames:
        return pd.DataFrame()

    pnl_df = pd.concat(pnl_frames, axis=1).fillna(0.0)
    total_capital = capital_per_pair * pnl_df.shape[1]

    portfolio = pd.DataFrame(index=pnl_df.index)
    portfolio["daily_pnl"] = pnl_df.sum(axis=1)
    portfolio["equity"] = total_capital + portfolio["daily_pnl"].cumsum()
    portfolio["daily_return"] = portfolio["daily_pnl"] / total_capital
    portfolio["cumulative_return"] = portfolio["equity"] / total_capital - 1.0
...
def show_strategy2_result(result: dict[str, object], config: CointegrationResearchConfig) -> None:
    ...
    portfolio: pd.DataFrame = result["portfolio"]  # type: ignore[assignment]
    ...
    if not portfolio.empty:
        ...
        daily_ret = portfolio["daily_return"].replace([np.inf, -np.inf], np.nan).dropna()
        sharpe_value = np.nan
        if len(daily_ret) > 2 and daily_ret.std(ddof=1) > 0:
            sharpe_value = float(np.sqrt(252) * daily_ret.mean() / daily_ret.std(ddof=1))

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Portfolio Total Return", pct(total_return))
        c2.metric("Portfolio Sharpe", num(sharpe_value))
        c3.metric("Portfolio Max DD", pct(max_dd))
        c4.metric("Backtested pairs", f"{len(summary)}")

        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=portfolio.index,
            y=portfolio["cumulative_return"],
            name="Strategy2 Portfolio",
            mode="lines",
        ))
        fig.update_layout(
            title="Strategy2 Portfolio Cumulative Return",
        ...
        fig.update_layout(
            title="Strategy2 Portfolio Drawdown",
            template="plotly_white",
            height=420,
            yaxis_tickformat=".1%",
            hovermode="x unified",
        ...
        st.warning("有篩選到 pair，但 test period 沒有產生可回測的 equity curve。")
